...
<title>In Process Request Details Reassign</title>
...
function validate_form()
{
	...
	{
		alert("Please Select Reassign.");
		document.forms['form1'].Elements['Status'].focus();
		return false;
	}
...
<!-- Form starts -->
<form name ='form1' action='reassigninprocessrequest.php?reqId=$Id' method='POST' onSubmit='javascript: return validate_form();'>
...
<font face='Tahoma' size='2' color='B40486'>Select whom to Reassign, 
then click on<i> Reassign </i> to Reassign the request to 'Reassign To'. To get back the original values, click <i> Reset</i>.</font></center>
</td></tr>
</table>
<br></br>
...
<tr>
  <td align='right'><b><font face='Tahoma' size='2' color='44778D'>Reassign To:</font></b></td>
  <td align='left'>
                <select size='1' name='Status'>  		
		<option></option>";
		
	$ddQuery = mysql_query('SELECT * FROM userdata WHERE (UserGroup="Admin" OR UserGroup="Super User") ORDER BY Name');	

 while ($dropdownitems = mysql_fetch_array($ddQuery))
 {
	If($dropdownitems['Email']==@$AssignedTo)
		{echo "<option selected value='$dropdownitems[Email]'>$dropdownitems[name]($dropdownitems[Email])</option>";}
	Else
        {echo "<option value='$dropdownitems[Email]'>$dropdownitems[name]($dropdownitems[Email])</option>";}
  }  
     echo "</select></td>
</tr>
<tr>
...
<tr>                
		...
		<button type='Submit' name='Submit' id='Submit' onClick="document.getElementById('soundfile').play();">Reassign</button>
		<button type='Reset' name='Reset' id='Reset' style='width:100px;' onClick="document.getElementById('soundfile').play();">Reset</button>
<?php 	echo "	</td>
</tr>
</table>
</form>
