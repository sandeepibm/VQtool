...
<title>Escalate In-Process Request</title>
...
<?php
  ...
  $EscalatedTo =  $_POST["email"];  
  $Remarks=nl2br($_POST["Description"]);  
  $EscalatedToName = substr($EscalatedTo,0,strrpos($EscalatedTo,"(")-1);
  $EscalatedToEmail = substr($EscalatedTo,strrpos($EscalatedTo,"(")+1,-1);  
  $Id = $_GET["reqId"];
...
<tr><td  height='23'>
<b><center>Escalate In-Process Request</font></center></b>
</td></tr>
</table>
<br></br>
...
<?php
      ...
      if (!$EscalatedTo) {
          echo "<center><font face='Verdana' Size='2' color='FF0000'>No Escalation Manager Set yet.</font></center>";
      }
	  echo "<br><br>";
...
*/	
      mysql_query("UPDATE allrequests SET Escalated = 'Y', EscalationRemarks='".mysql_real_escape_string($Remarks)."' WHERE RequestId='$Id'");
		...
		// for Assigned to
		$query_Assigned=mysql_query("Select * From userdata Where Email='$AssignedTo'");
		$infoAssigned=mysql_fetch_array($query_Assigned) ;
		$AssignedToName=$infoAssigned['name'];
...
*/	
	  ...
	  echo "<table width='55%' border='0' align='center' valign='middle'>
			<tr><td  align='center'>";			
	  echo "<label><font face='Verdana' Size='2'>Request has been successfully escalated to <b><font color='44778D'>$EscalatedTo</font></b>.</font>";
	  echo "</td></tr>
			</table>";
...
$mailmsg = new Mail_mime();
	...
	$message = '<HTML>
				...
				<table id="table4" width="65%" style="border:3px solid #cccccc;background:#C7F8B7;">
					...
					<tr>
					 <td valign="top">
					 <p><font color="B40486">Remarks :</font></p>
					 </td>
					 <td>
					 <p><font color="B40486">'.$Remarks.'</font></p>
					 </td>					 
	...
	$message = '<HTML>
				...
				<table id="table4" width="65%" style="border:3px solid #cccccc;background:#C7F8B7;">
					...
					<tr>
					 ...
					 <td>
					 <p><font color="#B40486"> '.$SeverityDesc.' days</font></p>
					 </td>				 
					 </tr>
...
$mail = Mail::factory("mail");
	...
	echo "<script type='text/javascript'>progressBarSim(61,80,'Sending Email to the Assigned Person...','Email ".htmlspecialchars($progMsg, ENT_QUOTES )." sent to the Escalation Manager.',".$interval.",'".$prevColors."','prevMessage3');</script>";
	try 
	{
		while (ob_get_level() > 0)
		ob_end_flush();
...
$mailmsg = new Mail_mime();
	$progMsg='';
	$to=$AssignedTo;
	$subject = 'Request No - '.$Id.' Escalated from You in VQ portal!';
	$message = '<HTML>
				<BODY>
				...
				<table id="table4" width="65%" style="border:3px solid #cccccc;background:#C7F8B7;">
					<tr>
					 <td colspan="2">
					 <p><font color="0A5DAB">Please Note: Request No-'.$Id.' has been escalated from you to <font color="#07B7B5">'.$EscalatedToName.'(<font color="#000000"><b>'.$EscalatedToEmail.'</b></font>) </font>.</font></p>
					 </td>					 
					</tr>
...
$mail = Mail::factory("mail");
	...
	echo "<script type='text/javascript'>progressBarSim(81,100,'Process Completed','Email ".htmlspecialchars($progMsg, ENT_QUOTES )." sent to the Assigned Person.',".$interval.",'".$prevColors."','prevMessage3');</script>";
	try 
	{
		while (ob_get_level() > 0)
		ob_end_flush();
