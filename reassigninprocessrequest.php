...
<title>Reassign In-Process Request</title>
...
<?php
  ...
  $ReassignedTo =  $_POST["Status"];         // Name
  $Id = $_GET["reqId"];
...
<tr><td  height='23'>
<b><center>Reassign In-Process Request</font></center></b>
</td></tr>
</table>
<br></br>
...
<?php
      ...
      if (!$ReassignedTo) {
          echo "<center><font face='Verdana' Size='2' color='FF0000'>Please select to Reassign./font></center>";
      }
	  echo "<br><br>";
...
*/
      ...
      mysql_query("UPDATE allrequests SET AssignedTo='$ReassignedTo',UpdatedOn=Now(),UpdatedBy='$namecookie' WHERE RequestId='$Id'");
...
*/	
	  ...
	  echo "<table width='55%' border='0' align='center' valign='middle'>
			<tr><td  align='center'>";			
	  echo "<label><font face='Verdana' Size='2'>Request has been successfully reassigned to $ReAssignedToName (<b><font color='44778D'>$ReassignedTo</font></b>).</font>";
	  echo "</td></tr>
			</table>";
...
<tr><td align='center'>
	<h1 id="finalMessage" style="font-size: 12px; color:#000000; display:inline-block">Reassigning Request in the system...</h1>
</tr></td>
<tr><td align='center'>
...
<tr><td align='center'>
	<h1 id="prevMessage4" style="font-size: 12px; display:inline-block">&nbsp;</h1>
</tr></td>
</table>	
...
<?php
	...
	echo "<script type='text/javascript'>progressBarSim(0,20,'Sending Email to the Requestor...','Request Reassigned in the system.',".$interval.",'".$prevColors."','prevMessage1');</script>";
	try 
	{
...
$mailmsg = new Mail_mime();
	...
	$progMsg='';
	$to=$Email;
	$subject = 'Your Request No - '.$Id.' has been Reassigned to '.$ReAssignedToName.' in VQ portal!';
	$message = '<HTML>
				<BODY>
				...
				<table id="table4" width="65%" style="border:3px solid #cccccc;background:#C7F8B7;">
					<tr>
					 <td colspan="2">
					 <p><font color="0A5DAB">Please Note - Your request has been reassigned from <font color="#07B7B5">RSC OPS SPOC - '.$AssignedToName.'(<font color="#000000"><b>'.$AssignedTo.'</b></font>) </font>to <font color="#07B7B5">RSC OPS SPOC - '.$ReAssignedToName.'(<font color="#000000"><b>'.$ReassignedTo.'</b></font>)</font>.</font></p>
					 </td>					 
					</tr>
...
$mail = Mail::factory("mail");
	...
	echo "<script type='text/javascript'>progressBarSim(21,60,'Sending Email to the Reassigned Person...','Email ".htmlspecialchars($progMsg, ENT_QUOTES )." sent to the Requestor.',".$interval.",'".$prevColors."','prevMessage2');</script>";
	try 
	{
		while (ob_get_level() > 0)
		ob_end_flush();
...
$mailmsg = new Mail_mime();
	$progMsg='';
	$to=$ReassignedTo;
	$subject = 'Request No - '.$Id.' Reassigned to You in VQ portal!';
	$message = '<HTML>
				<BODY>
				<font face="Tahoma" size="2" color="44778D">Dear '.$ReAssignedToName.',</font>
				<br></br>

				<table id="table4" width="65%" style="border:3px solid #cccccc;background:#C7F8B7;">
					<tr>
					 <td colspan="2">
					 <p><font color="0A5DAB">Please Note - you\'ve been assigned to the Request No-'.$Id.'</font></p>
					 </td>					 
					</tr>

					<tr>
					 <td colspan="2">
					 <p><font color="#07B7B5">You\'re Reassigned to the request from the RSC OPS SPOC '.$AssignedToName.'(<font color="#000000"><b>'.$AssignedTo.'</b></font>)</font></p>
					 </td>					 
					</tr>
	...
	catch( Exception $e ) 
	{
		$mailsent=false;
			...
			{echo"<table border='0' width='63%' align='right' valign='top'>
					<tr><td>
					<font face='Tahoma' size='2' color='06B2B0' style='background:#ffffff'>Email sent to the Reassigned person's registered email id(".$to.") with all information.</font>
					</td></tr>
					</table>";}
			else
			{echo "<table border='0' width='64%' align='right' valign='top'>
					<tr><td>
					<font face='Tahoma' size='2' color='FA8201' style='background:#ffffff'>Problems occured.Couldn't send email to the Reassigned person's registered email id(".$to.").</font>
					</td></tr>
					</table>";
					}

	echo "<script type='text/javascript'>progressBarSim(61,80,'Sending Email to the previously assigned Person...','Email ".htmlspecialchars($progMsg, ENT_QUOTES )." sent to the Reassigned Person.',".$interval.",'".$prevColors."','prevMessage3');</script>";
	try 
	{
		while (ob_get_level() > 0)
		ob_end_flush();
...
$mailmsg = new Mail_mime();
	$progMsg='';
	$to=$AssignedTo;
	$subject = 'You\'re released from the Request No - '.$Id.' in VQ portal!';
	$message = '<HTML>
				<BODY>
				...
				<table id="table4" width="65%" style="border:3px solid #cccccc;background:#C7F8B7;">
					<tr>
					 <td colspan="2">
					 <p><font color="0A5DAB">Please Note - you\'ve been released from the Request No-'.$Id.'</font></p>
					 </td>					 
					</tr>

					<tr>
					 <td colspan="2">
					 <p><font color="#07B7B5">Request is reassigned to RSC OPS SPOC '.$ReAssignedToName.'(<font color="#000000"><b>'.$ReassignedTo.'</b></font>)</font></p>
					 </td>					 
					</tr>
...
$mail = Mail::factory("mail");
	...
	catch( Exception $e ) 
	{
		$mailsent=false;
			...
			{echo"<table border='0' width='63%' align='right' valign='top'>
					<tr><td>
					<font face='Tahoma' size='2' color='06B2B0' style='background:#ffffff'>Email sent to the previously assigned person's registered email id(".$to.") with all information.</font>
					</td></tr>
					</table>";}
			else
			{echo "<table border='0' width='64%' align='right' valign='top'>
					<tr><td>
					<font face='Tahoma' size='2' color='FA8201' style='background:#ffffff'>Problems occured.Couldn't send email to the previously assigned person's registered email id(".$to.").</font>
					</td></tr>
					</table>";
					}
 

	echo "<script type='text/javascript'>progressBarSim(81,100,'Process Completed.','Email ".htmlspecialchars($progMsg, ENT_QUOTES )." sent to the previously assigned Person.',".$interval.",'".$prevColors."','prevMessage4');</script>";
	try 
	{
		while (ob_get_level() > 0)
		ob_end_flush();
