...
<?php
  ...
  $sql = mysql_query("SELECT * FROM userdata WHERE concat(Name,'(',Email,')')='".mysql_real_escape_string($UsrAdm)."'");
...
<tr><td  height='23'>
<b><center>Update Manager</font></center></b>
</td></tr>
</table>
<br></br>
...
<?php
	  if ((!$email)||(!$UsrAdm)||(!$Id)) {
		...
		echo "<table width='60%' border='0' align='right' valign='top'>
				<tr><td>
				<font face='Tahoma' size='2' color='FF0000'>Please Enter Manager's Email.</font>
				</td></tr>
			</table>";
		  }
		  if (!$UsrAdm) {
		echo "<table width='60%' border='0' align='right' valign='top'>
				<tr><td>
				<font face='Tahoma' size='2' color='FF0000'>Please Select Assigned Admin.</font>
				</td></tr>
			</table>";
		  }
		  if (!$Id) {
...
*/
	  ...
	  $seqinuse = mysql_query("SELECT * FROM tblmanager WHERE Email='".mysql_real_escape_string($email)."' and MgrId<>$Id");
	  $isseqinuse = mysql_num_rows($seqinuse);
	  // If sequence number exists then print error message and exit script
	  If ($isseqinuse == 1) {
		  echo "<center><font face='Verdana' Size='2' color='FF0000'>The Email you entered is already occupied by a Manager.</font></center>";
		  echo "<br></br><br></br><br></br>";
		  echo "<a href='javascript:history.back()'><center><font size='3'>Back</font></center></a>";
		  exit;
		}
      ...
      mysql_query("UPDATE tblmanager SET Email='".mysql_real_escape_string($email)."', AsgndAdmUsrId='".mysql_real_escape_string($usrAdmId)."' WHERE MgrId=$Id");

      // Print Successful Creation of user message

	  echo "<table border='0' align='center' valign='middle'>
			<tr><td  align='center'>
			<b><font face='Verdana' Size='2' color='848484'>Manager has been updated successfully.</font></b>
			</td></tr>
			</table>";
	  echo "<br><br><br><br>";
